...
const FEATURED: Featured[] = [
    {
        ...
        problem:
            "단일 2D 이미지에서 3D를 복원하는 것은 깊이 모호성(depth ambiguity) 때문에 ill-posed 문제입니다. 여러 접근을 순차적으로 시도하며 한계를 직접 확인했습니다 — (1) 대칭 형상 객체(의자·침대·테이블)를 세그멘테이션한 뒤 2장의 사진을 픽셀 단위로 매칭·교차(intersection)하고 이미지에서 텍스처를 투영하는 자체 설계 멀티뷰 방식(깊이 정확도에 근본적 한계), (2) Occupancy Network(대칭이 없는 형상·커스텀 데이터에서 일반화 부족), (3) NeRF 및 '뷰를 먼저 생성한 뒤 3D로 복원'하는 image-based 방식(품질은 확인했으나 장면별 최적화 시간과 제약이 큼).",
        hypothesis:
