...
### 배경 제거
- U2Net을 세밀하게 조정하여 의류 이미지의 배경을 제거하고, 깨끗하고 선명한 시각적 데이터를 생성했습니다.
- 다양한 배경을 포함한 의류 이미지 데이터셋을 활용하여 모델 학습을 진행했습니다.
- 노이즈와 아티팩트를 최소화하여 고품질의 세분화 마스크를 생성했습니다.
...
### 의류 부위 분할
- U2Net을 확장하여 의류를 세 가지 주요 카테고리로 분할했습니다:
  - **상의(Top)**: 셔츠, 블라우스, 재킷 등
  - **하의(Bottom)**: 바지, 치마, 반바지 등
  - **원피스(Dress)**: 전체 길이의 드레스
- 정밀한 주석 데이터를 활용하여 가장자리 및 질감 디테일을 개선하여 모델 정확도를 향상했습니다.
...
### 모델 최적화
- 다양한 데이터셋을 활용하여 모델을 튜닝하여 다음과 같은 변수에 대응하도록 개선했습니다:
  - 다양한 의류 질감(예: 단색, 패턴, 반투명)
  - 조명 및 그림자 변화
  - 포즈 및 의류 겹침
- 모델이 다양한 스타일의 의류에서도 일반화될 수 있도록 새로운 데이터에 대한 성능 검증을 진행했습니다.
